...
public class TestTwoStackExpressionEvaluation {
	...
	public static void postfixTest() throws Exception {
		...
		String[] testCases = { "(-sin(-x))^(-10-x)", "(z-x)-(y-(z-1)^(4-x))^(y-z)", "sin(cos(x+exp(x-z))+ln(x))",
				"(7-x-5)/z/y*z*x+7+8-9", "7^z^y^2/z^x^3" };
		String[] tokenized = { " x _ sin _ 10.0 _ x - ^", " z x - y z 1.0 - 4.0 x - ^ - y z - ^ -",
				" x x z - exp + cos x ln + sin", " 7.0 x - 5.0 - z / y / z * x * 7.0 + 8.0 + 9.0 -",
				" 7.0 z y 2.0 ^ ^ ^ z x 3.0 ^ ^ /" };
	...
	public static void evaluationTest() throws Exception {
		...
		String[] testCases = { "(-sin(-x))^(-10-x)", "(z-x)-(y-(z-1)^(4-x))^(y-z)", "sin(cos(x+exp(x-z))+ln(x))",
				"(7-x-5)/z/y*z*x+7+8-9", "7^z^y^2/6^z^4^x" };
